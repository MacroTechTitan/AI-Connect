...
- Sprint 4 shipped (PR #8 merge 2026-06-05) — Project Genesis MVP. Schema migration (`platform_credentials`, `project_provisioning_events`, projects `provisioning_state`), platform integration layer (Vercel/Render/GitHub/Supabase clients), platform-credential endpoints + UI, orchestrator core (5a), best-effort reverse-order rollback (5b), and SSE streaming + frontend genesis UI (5c). Migrations 0003 and 0004 applied to Supabase.
- Sprint 5 in progress on branch sprint/5-genesis-completion — schema migration (0005, applied), Cloudflare client + GitHub create-from-template (Commit 2), Supabase wait+capture (Commit 3), orchestrator integration (Commit 4 — template choice routing, real DNS provisioning, real env var injection, extended rollback for Cloudflare and Render env vars), project creation UX with template selection + subdomain display (Commit 5), and these docs (Commit 6). A strategic thinking sketch (`docs/architecture/tool-routing-and-skills.md`) was captured mid-Sprint 5 — forward-looking tool-routing/composable-skills notes, not Sprint 5 scope.
- Production currently runs Sprint 4 from master. Sprint 5 ships to production once this branch merges per the MTTBuild merge-and-ship ritual, followed by a smoke test against the live URLs (see `docs/sprints/SPRINT_5_TESTING.md`).
